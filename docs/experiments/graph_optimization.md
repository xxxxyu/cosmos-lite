...
boundary. Sampler timestep transfers remain, but profiling showed too few of
them to justify an invasive packing API rewrite.
...
This small sample verifies the end-to-end server and RoboLab integration.
Success-rate comparisons use exact replay32 parity for shared quantization and
the 50-episode compile comparison above.
