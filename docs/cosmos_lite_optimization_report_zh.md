...
在统一的 RoboLab Banana、guidance 3、2 denoise steps、32x8 action chunk
协议下，当前最快且通过 50-episode 闭环门控的结果是：

| Model            | 当前配置                                                                              | RTX 4090 request p50 | Peak reserved | Banana success rate |
| ---------------- | ------------------------------------------------------------------------------------- | -------------------: | ------------: | ------------------: |
...
| -------------------------- | ------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------- |
| NVIDIA 原始框架            | CloseFridge BF16 加载 31.76GB、峰值 33.09GB，无 4090 baseline | BF16 g3/s2 为 582ms、9.20GB、34/50    | 能训练和推理，但不是 4090 Nano 部署方案                   |
| Cosmos Lite 首个稳定量化版 | Full W8 g3/s2 为 2403ms、21.42GB、45/50                       | Full W8 g3/s2 为 576ms、8.71GB、36/50 | 自包含 W4/W8 bundle，一键 replay/rollout                  |
| 当前最快优化版             | 958.5ms、15.51GB、49/50                                       | 331.4ms、8.79GB、40/50                | FP8、compile、attention/cache、SM89 kernel 和数据路径优化 |
...
这里需要特别区分协议：Nano 原始 BF16 显存来自 RoboCasa CloseFridge，而
当前最快 Nano 延迟和 SR 来自 RoboLab Banana 上的 DROID policy。跨环境表格
用于说明部署能力如何演进，不代表严格的同协议加速比；上面的 2.51x 使用的是
同为 RoboLab 的 Full W8 baseline。

这些 SR 点估计来自 50 个 paired initial states。它们适合检查明显退化，不足以
证明几个百分点的提升具有普遍统计意义。
...
但 rollout 从 49/50 降到 47/50。虽然 50 episodes 不能证明统计显著退化，
保守策略仍不推广；Nano allowlist 明确排除这些 shape，继续使用 CUTLASS。
