...
Under the same RoboLab Banana protocol, guidance 3, two denoise steps, and a
32x8 action chunk, the fastest configurations that passed a paired 50-episode
closed-loop gate are:

| Model            | Current configuration                                                                 | RTX 4090 request p50 | Peak reserved | Banana success rate |
| ---------------- | ------------------------------------------------------------------------------------- | -------------------: | ------------: | ------------------: |
| Cosmos3 Edge 4B  | GenW8A8 + compile + shared FP8 + Triton SM89 + Sage FP16-PV + sparse transform        |         **331.4 ms** |       8.79 GB |     **40/50 = 80%** |
| Cosmos3 Nano 16B | GenW8A8 + compile + shared FP8 + Sage FP8-PV + condition K/V cache + sparse transform |         **958.5 ms** |      15.51 GB |     **49/50 = 98%** |
...
These diagnostics are useful for screening. They do not replace rollout: a
small action difference may grow through closed-loop interaction, or may have
...
| ------------------------------------------ | -------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------ |
| Original NVIDIA framework                  | CloseFridge BF16 loaded at 31.76GB and peaked at 33.09GB; no 4090 baseline | BF16 g3/s2: 582ms, 9.20GB, 34/50    | Training and inference worked, but Nano was not a 4090 deployment  |
| First stable Cosmos Lite quantized release | Full W8 g3/s2: 2403ms, 21.42GB, 45/50                                      | Full W8 g3/s2: 576ms, 8.71GB, 36/50 | Self-contained W4/W8 bundles and one-command replay/rollout        |
| Current fastest optimization branch        | 958.5ms, 15.51GB, 49/50                                                    | 331.4ms, 8.79GB, 40/50              | FP8, compile, attention/cache, SM89 kernels, and data-path cleanup |
...
policy on RoboLab Banana. That cross-environment row documents the deployment
journey; it is not a same-protocol speedup claim. The 2.51x Nano speedup above
uses the matched RoboLab Full W8 baseline.

The success-rate point estimates use 50 paired initial states. They can catch
clear regressions, but they do not establish that a difference of a few points
will generalize to every task or robot.
...
Machete requires SM90/Hopper and is not a 4090 backend. ExLlamaV2/V3 is useful
evidence for consumer-GPU inference, but its runtime and LLM assumptions did
...
ranking. The product therefore retained four simpler strategies with broader
evidence.
...
This does not accelerate a kernel, but it fixed the serious early risk that a
supposedly quantized model still depended on its source checkpoint.
...
reduction, but rollout moved from 49/50 to 47/50. The sample does not prove a
statistically significant regression, but it also does not justify replacing
a 98% profile. Nano shapes are excluded from the Triton allowlist and remain on
CUTLASS.
...
### 13.1 Current fastest experimental profiles
...
- [Nano RoboLab benchmark](../examples/robolab_quant/NANO_BENCHMARKS.md)
- [Edge RoboLab benchmark](../examples/robolab_quant/EDGE_BENCHMARKS.md)
- [RoboCasa365 benchmark](../examples/robocasa365_quant/BENCHMARKS.md)
- [FP8 W8A8 experiment](../examples/robolab_quant/FP8_W8A8_EXPERIMENT.md)
- [Graph optimization experiment](../examples/robolab_quant/GRAPH_OPTIMIZATION_EXPERIMENT.md)
- [RTX 4090 SM89 optimization](../examples/robolab_quant/SM89_OPTIMIZATION.md)
