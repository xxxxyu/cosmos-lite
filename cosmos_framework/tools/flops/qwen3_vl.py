...
Usage:
    from cosmos_framework.tools.flops.qwen3_vl import compute_qwen3vl_flops

    # total_tokens is the packed sequence length (text + visual) fed to
    # the text decoder. visual_tokens is the post-vision-encoder count
    # after spatial merging. num_patches (optional) is the pre-merging
    # patch count used to size the vision encoder's own work; for Qwen3-VL
    # with default spatial_merge_size=2 it is visual_tokens * 4. Pass
    # num_patches=None to skip the vision-encoder term.
...
def compute_text_decoder_flops(
    ...
    Args:
        ...
        is_causal: If True (default), apply the causal-attention factor of 0.5
            to the S^2 terms in self-attention. Qwen3-VL text decoders are
...
def compute_qwen3vl_flops_from_config(
    ...
    """Compute FLOPs using a HuggingFace Qwen3VL config object (supports MoE).

    Args:
        config: HuggingFace transformers config for the Qwen3-VL family,
            i.e. a Qwen3VLConfig (dense) or Qwen3VLMoeConfig (MoE) instance
            obtained from AutoConfig.from_pretrained(...). The function reads
            the nested attributes config.text_config and config.vision_config
            using the field names defined by the upstream HF schema.
        total_tokens: Total number of tokens in sequence (text + visual).
        visual_tokens: Number of visual tokens (after vision encoder processing).
        num_patches: Number of patches (pixel_values.shape[0] or
            image_grid_thw.prod(-1)). Pass None to skip the vision-encoder
            FLOPs.
        is_causal: If True (default), the text decoder's S^2 attention terms
            are halved to account for causal masking; the vision encoder is
            always treated as bidirectional. Pass False to recover the
            bidirectional upper bound (e.g. when feeding into a runtime
            estimator whose curve was fit against the upper bound).
