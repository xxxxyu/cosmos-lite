...
from cosmos_framework.utils.functional.batch_ops import batch_mul
...
def res_x0_rk2_step(
    ...
    """
    s = -torch.log(s)  # scalar or [B]
    t = -torch.log(t)  # scalar or [B]
    m = -torch.log(s1)  # scalar or [B]
    ...
    c2 = (m - s) / dt  # scalar or [B]
    phi1_val, phi2_val = phi1(-dt), phi2(-dt)  # scalar or [B] each

    # Handle edge case where t = s = m
    b1 = torch.nan_to_num(phi1_val - 1.0 / c2 * phi2_val, nan=0.0)  # scalar or [B]
    b2 = torch.nan_to_num(1.0 / c2 * phi2_val, nan=0.0)  # scalar or [B]

    return batch_mul(torch.exp(-dt), x_s) + batch_mul(dt, batch_mul(b1, x0_s) + batch_mul(b2, x0_s1))  # [B,...]
...
def reg_x0_euler_step(
    ...
    """
    coef_x0 = (s - t) / s  # scalar or [B]
    coef_xs = t / s  # scalar or [B]
    return batch_mul(coef_x0, x0_s) + batch_mul(coef_xs, x_s), x0_s  # [B,...], [B,...]
...
def reg_eps_euler_step(
    ...
    """
    return x_s + batch_mul(eps_s, t - s), x_s + batch_mul(eps_s, 0 - s)  # [B,...], [B,...]
...
def rk1_euler(
    ...
    """
    x0_s = x0_fn(x_s, s)  # [B,...]
    return reg_x0_euler_step(x_s, s, t, x0_s)  # [B,...], [B,...]
...
def rk2_mid_stable(
    ...
    Returns:
        Tuple[Tensor, Tensor]: Updated state tensor and x0 prediction.
    """
    s1 = torch.sqrt(s * t)  # scalar or [B]
    x_s1, _ = rk1_euler(x_s, s, s1, x0_fn)  # [B,...]

    x0_s1 = x0_fn(x_s1, s1)  # [B,...]
    return reg_x0_euler_step(x_s, s, t, x0_s1)  # [B,...], [B,...]
...
def rk2_mid(x_s: torch.Tensor, s: torch.Tensor, t: torch.Tensor, x0_fn: Callable) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    s1 = torch.sqrt(s * t)  # scalar or [B]
    x_s1, x0_s = rk1_euler(x_s, s, s1, x0_fn)  # [B,...], [B,...]

    x0_s1 = x0_fn(x_s1, s1)  # [B,...]

    return res_x0_rk2_step(x_s, t, s, x0_s, s1, x0_s1), x0_s1  # [B,...], [B,...]
...
def rk_2heun_naive(
    ...
    """
    x_t, x0_s = rk1_euler(x_s, s, t, x0_fn)  # [B,...], [B,...]
    eps_s = batch_mul(1.0 / s, x_t - x0_s)  # [B,...]
    x0_t = x0_fn(x_t, t)  # [B,...]
    eps_t = batch_mul(1.0 / t, x_t - x0_t)  # [B,...]

    avg_eps = (eps_s + eps_t) / 2  # [B,...]

    return reg_eps_euler_step(x_s, s, t, avg_eps)  # [B,...], [B,...]
...
def rk_2heun_edm(
    ...
    avg_x0 = (x0_s + x0_t) / 2  # [B,...]

    return reg_x0_euler_step(x_s, s, t, avg_x0)  # [B,...], [B,...]
...
def rk_3kutta_naive(
    ...
    Returns:
        Tuple[Tensor, Tensor]: Updated state tensor and current state.
    """
    c2, c3 = 0.5, 1.0  # stage time fractions
    a31, a32 = -1.0, 2.0  # Butcher tableau coefficients
    b1, b2, b3 = 1.0 / 6, 4.0 / 6, 1.0 / 6  # quadrature weights

    delta = t - s  # scalar or [B]

    s1 = c2 * delta + s  # scalar or [B]
    s2 = c3 * delta + s  # scalar or [B]
    x_s1, x0_s = rk1_euler(x_s, s, s1, x0_fn)  # [B,...], [B,...]
    eps_s = batch_mul(1.0 / s, x_s - x0_s)  # [B,...]
    x0_s1 = x0_fn(x_s1, s1)  # [B,...]
    eps_s1 = batch_mul(1.0 / s1, x_s1 - x0_s1)  # [B,...]

    _eps = a31 * eps_s + a32 * eps_s1  # [B,...]
    x_s2, _ = reg_eps_euler_step(x_s, s, s2, _eps)  # [B,...]

    x0_s2 = x0_fn(x_s2, s2)  # [B,...]
    eps_s2 = batch_mul(1.0 / s2, x_s2 - x0_s2)  # [B,...]

    avg_eps = b1 * eps_s + b2 * eps_s1 + b3 * eps_s2  # [B,...]
    return reg_eps_euler_step(x_s, s, t, avg_eps)  # [B,...], [B,...]
