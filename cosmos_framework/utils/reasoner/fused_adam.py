...
from cosmos_framework.utils.reasoner import distributed
...
class FusedAdam(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        # Skip buffer
        self._dummy_overflow_buf = torch.tensor([0], dtype=torch.int, device="cuda")  # [1]
        self.multi_tensor_adam = te.pytorch.optimizers.multi_tensor_adam
        self.multi_tensor_adam_capturable = te.pytorch.optimizers.multi_tensor_adam_capturable
        self.multi_tensor_adam_capturable_master = te.pytorch.optimizers.multi_tensor_adam_capturable_master
        ...
        for group, group_master in zip(self.param_groups, self.param_groups_master):
            ...
            for p, p_master in zip(group["params"], group_master["params"]):
                ...
                if len(state) == 0:
                    ...
                    state["exp_avg"] = torch.zeros_like(p).float()  # [*p.shape]
                    # Exponential moving average of squared gradient values
                    # Change that makes .step() not crash
                    state["exp_avg_sq"] = torch.zeros_like(p).float()  # [*p.shape]
            ...
            if self.capturable:
                ...
                found_inf = (
                    ...
                    else torch.zeros((1,), device=device)  # [1]
                )
                ...
                if grad_scaler:
                    ...
                    scale = torch.ones((1,), device=device, dtype=torch.float32)  # [1]
                    inv_scale = torch.ones((1,), device=device, dtype=torch.float32)  # [1]
    ...
    def load_state_dict(self, state_dict):
        ...
        for group in self.param_groups:
            ...
            if "step" in group:
                if self.capturable:
                    if distributed.get_rank() == 0:
                        ...
                        step = torch.zeros(1, dtype=torch.int32).cuda()  # [1]
            ...
            for p in group["params"]:
                ...
                assert p.shape == state["exp_avg"].shape, (
                    f"p.shape: {p.shape} state['exp_avg'].shape: {state['exp_avg'].shape}"
                )
                assert p.shape == state["exp_avg_sq"].shape, (
                    f"p.shape: {p.shape} state['exp_avg_sq'].shape: {state['exp_avg_sq'].shape}"
                )
